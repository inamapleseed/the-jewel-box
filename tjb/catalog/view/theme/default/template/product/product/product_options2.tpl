    ...
    <div class="form-group">
      ...
      <div class="btns">
        <?php if(!$not_avail && !$enquiry) { ?>
            <button type="button" onclick="cart.add('<?= $product_id; ?>', '<?= $minimum; ?>');" class="btn btn-primary"> ADD TO CART </button>
        <?php } ?>

        <?php if($not_avail) { ?>
          <button type="button" id="button-cart" data-loading-text="<?= $text_loading; ?>" class="btn btn-primary" style="background: transparent; opacity: .4" disabled>SOLD OUT</button>
        <?php } ?>
          
            <button type="button" onclick="enquiry.add('<?= $product_id; ?>', '<?= $minimum; ?>');" class="btn btn-primary enq">ADD TO ENQUIRY</button>
      </div>
