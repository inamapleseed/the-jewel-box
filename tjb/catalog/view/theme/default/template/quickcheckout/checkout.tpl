...
<div class="container">
  ...
  <div class="row"><?php // $column_left; // use for social login  ?>
    <?php if ($column_right) { ?>
    <?php $class = 'col-sm-6'; ?>
    ...
    <div id="content" class="<?php echo $class; ?>">
	  ...
	  <link rel="stylesheet" media="screen and (min-width: 992px) and (max-width: 99999px)" href="<?php echo $stylesheet; ?>" />
	  <link rel="stylesheet" media="screen and (min-width: 1px) and (max-width: 991px)" href="<?php echo $mobile_stylesheet; ?>" />
	  <?php } else { ?>
	  ...
	  <div id="quickcheckoutconfirm">
		<div id="quickcheckout-disable">
		  ...
		  <div class="quickcheckoutmid" id="login-box">
			...
			<?php /*<div class="or"><?php echo $text_or; ?></div>*/ ?>
		  </div>
		  <?php } ?>
		  <div class="quickcheckoutleft" style="overflow:visible;">
...
$(window).load(function() {
    $.blockUI({
		message: '<h1 style="color:#ffffff;"><?php echo $text_please_wait; ?></h2>',
		css: {
			border: 'none',
			padding: '15px',
			...
			if (json['redirect']) {
				...
				<?php if ($text_error) { ?>
					...
					if (json['error']['birthday']) {
						$('#payment-address input[name=\'birthday\']').after('<div class="text-danger">' + json['error']['birthday'] + '</div>');
					}
				<?php } ?>
	...
	$.ajax({
		...
		success: function(json) {
			if (json['redirect']) {
				...
				if(json['quantity']){
					$('#cart-quantity-total').text(json['quantity']);
				}
				<?php if (!$logged) { ?>
					if ($('#payment-address input[name=\'shipping_address\']:checked').val()) {
						reloadPaymentMethod();
...
<?php if (isset($pixel_tracking) ) { ?>
	<?= $pixel_tracking?>
<?php } ?>
<!-- FACEBOOK EVENT - INITIATECHECKOUT END -->
