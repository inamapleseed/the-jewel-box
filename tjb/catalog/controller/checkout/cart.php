...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$upsell_status = $this->config->get('upsell_product_addon_status');
			if(isset($upsell_status) && $upsell_status == 1){
				$data['upsell_status'] = true;
			}
			if($data['upsell_status']){
				...
				//Get module helper value
				$this->load->library('modulehelper');
				$Modulehelper = Modulehelper::get_instance($this->registry);
				$oc = $this;
				$language_id = $this->config->get('config_language_id');
				$modulename  = 'upsell_product_addon';
				$data['upsell_title'] = $Modulehelper->get_field ( $oc, $modulename, $language_id, 'title');
				$data['autoplay'] = $Modulehelper->get_field ( $oc, $modulename, $language_id, 'autoplay');
				
				if($data['autoplay']){
					$data['mobile_autoplay'] = $data['autoplay'];
				} else {
					$data['mobile_autoplay'] = '5000';
				}

				$related_results = array();
				$combine_related_results = array();

				foreach ($products as $product) {
					$related_results[] = $this->model_catalog_product->getProductRelated($product['product_id']);
				}
				
				$count = count($related_results);

				for($i=0;$i<$count;$i++){
					$combine_related_results = array_merge($combine_related_results,$related_results[$i]);
				}

				$combine_related_results = array_intersect_key($combine_related_results, array_unique(array_map('serialize', $combine_related_results)));
				

				foreach ($combine_related_results as $combine_related_result) {
					//put products2 coz products below used aldy
					$data['products2'][] = $this->load->controller('component/product_info', $combine_related_result['product_id']);
				}
			}
	...
	public function add() {
		...
		if ($product_info) {
			if (isset($this->request->post['quantity']) && ((int)$this->request->post['quantity'] >= $product_info['minimum'])) {
				...
				$quantity = (int)$this->request->post['quantity'];
				$json = array(
					'error_minstock_title'	=>	$this->language->get('error_minstock_title'),
					'error_minstock'		=>	sprintf($this->language->get('error_minstock'),$product_info['minimum']),
				);
				$min_stock_error = true;
			}
			...
			//Check option stock - Start
			//Rearrange post option 
			$new_option_array = array();
			foreach ($option as $opt_key => $opt_value) {
				if(is_array($opt_value)){
					foreach ($opt_value as $opt_inner_key => $opt_inner_value) {
						$new_option_array[$opt_key.'_'.$opt_inner_key] = $opt_inner_value;
					}
				} else {
					$new_option_array[$opt_key] = $opt_value;
				}
			}
			
			//Check normal option stock
			foreach ($new_option_array as $opt_key => $opt_value) {
				$tmp_key = explode('_',$opt_key);
				$new_key = $tmp_key[0];
				
				$product_option_found = array_search($new_key,array_column($product_options,'product_option_id'));
				if($product_option_found !== false){
					$product_option_value_found = array_search($opt_value,array_column($product_options[$product_option_found]['product_option_value'],'product_option_value_id'));
					if($product_option_value_found !== false){
						if(isset($product_options[$product_option_found]['product_option_value'][$product_option_value_found]['subtract']) && $product_options[$product_option_found]['product_option_value'][$product_option_value_found]['subtract'] == 1){
							if($quantity > $product_options[$product_option_found]['product_option_value'][$product_option_value_found]['quantity']){
								$json = array(
									'error_outofstock_title'	=>	$this->language->get('error_outofstock_title'),
									'error_outofstock'			=>	$this->language->get('error_outofstock'),
								);
			...
			//check related option stock
			if(isset($this->request->post['option'])){
				$this->load->model('module/related_options');
				$ro_settings = $this->config->get('related_options');
				$ro_for_product = $this->model_module_related_options->get_related_options_sets_by_poids($this->request->post['product_id'], $this->request->post['option'], true);
				
				if($ro_for_product){
					foreach($ro_for_product as $r_product){
						if($quantity > $r_product['quantity']){
							$json = array(
								'error_outofstock_title'	=>	$this->language->get('error_outofstock_title'),
								'error_outofstock'			=>	$this->language->get('error_outofstock'),
							);
			...
			if (!$json) {
				...
				$json['facebookevent_status'] = false;

				if($this->config->get('setting_facebookevents_status') == 1){
					$json['facebookevent_status'] = true;

					if($product_info['special']){
						$productprice = bcdiv($product_info['special'], 1, 2);
					}else{
						$productprice = bcdiv($product_info['price'], 1, 2);
					}
	
					$json['price'] = $productprice;
					$json['currency'] = $this->session->data['currency'];
					$json['content_type'] = "'product'";
					$json['content_ids'] = "['".$this->request->post['product_id']."']";
					$json['content_name'] = "'".$product_info['name']."'";
				}else{
					$json['facebookevent_status'] = false;
				}
