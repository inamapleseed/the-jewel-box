<?php
	class ControllerAccountLogin extends Controller {
		...
		public function index() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
				...
				if (isset($abandonedCartsSettings['abandonedcarts']['Enabled']) && $abandonedCartsSettings['abandonedcarts']['Enabled']=='yes') { 
					$this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts` WHERE `restore_id` = '".session_id()."' AND `ordered`=0");
					...
					if (!empty($cart)) {
						...
						if (empty($exists['customer_info'])) { 
							...
							$this->db->query("UPDATE `" . DB_PREFIX . "abandonedcarts` SET `customer_info` = '".$this->db->escape($customer)."', `restore_id`='".$this->customer->getEmail()."' WHERE `restore_id`='$id' AND `ordered`=0");
			...
			$data['text_register_now'] = sprintf($this->language->get('text_register_now'),$this->url->link('account/register', '', true)) ;
			$data['text_no_account'] = $this->language->get('text_no_account');
			$data['text_register_account'] = $this->language->get('text_register_account');
			$data['text_returning_customer'] = $this->language->get('text_returning_customer');
			$data['text_i_am_returning_customer'] = $this->language->get('text_i_am_returning_customer');
			$data['forgotten'] = sprintf($this->language->get('text_forgotten'), $this->url->link('account/forgotten', '', true), '') ;
		...
		protected function validate() {
			...
			// To be tested
			// if( !isset($this->request->post['password']) || utf8_strlen($this->request->post['password']) < 1  ){
			...
			if (!$this->error) {
				if (!$this->customer->login($this->request->post['email'], $this->request->post['password'])) {
					...
					$this->model_account_customer->getCustomerFailedLogin($this->request->post['email']);
