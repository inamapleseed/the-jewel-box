...
// 		}
	...
	public function index() {
		...
		*/
		...
		$birthday_field_class = 'birthday hidden';
		if($event_day){
			$birthday_field_class = 'birthday';
		}
		$cases = array(			
			...
			'confirm'		=>	array('type' => 'password', 	'parent_class' => '', 			'extra_class' => '', 				'list' => array()),			'birthday'		=>	array('type' => 'text',			'parent_class' => '', 			'extra_class' => $birthday_field_class,		'list' => array()),		
	...
	private function validate() {
		...
		if (utf8_strlen($this->request->post['unit_no']) < 1) 
			$this->error['unit_no'] = $this->language->get('error_unit_no');
		...
		if(isset($event_day_status) && $event_day_status == 1){
			if ((utf8_strlen(trim($this->request->post['birthday'])) < 1) || $this->request->post['birthday'] == '0000-00-00'){
				...
				if($event_day_age_validate == 1 && time() < strtotime('+18 years', strtotime($this->request->post['birthday']))){
					$this->error['birthday'] = $this->language->get('error_birthday_age');
				}
			}
		}
