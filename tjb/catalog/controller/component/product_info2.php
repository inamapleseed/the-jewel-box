<?php
    class ControllerComponentProductInfo2 extends Controller{
...
// End products or gallery
            ...
            $product_url = $this->url->link('product/product2', 'product_id=' . $product_info['product_id']);

            if ($url) {
                $product_url = $this->url->link('product/product2', $url . '&product_id=' . $product_info['product_id']);
            }
           ...
           $category = array();
            $main_cats = $this->model_catalog_product->getProductMainCategories($product_id);

            $this->load->model('catalog/category');
            foreach ($main_cats as $cat) {
                $category_info = $this->model_catalog_category->getCategory($cat['path_id']);
                if(isset($category_info['name'])){
                    if(!in_array($category_info['name'],$category)){
            ...
            return $this->load->view('component/product_info2', $info);
        }
    }