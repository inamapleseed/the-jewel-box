...
class ControllerExtensionModuleCategory extends Controller {
	...
	public function index() {
		...
		// debug($main_pcategories);
		
		if ($main_pcategories === 'masterpiece') {
			$route = 'product/masterpiece';
		} elseif ($main_pcategories === 'readytowear') {
			$route = 'product/ready-to-wear';	
		}  elseif ($main_pcategories === 'collection') {
	...
	private function getPriceFilter(){
		...
		if(isset($this->request->get['filter']) && isset($this->request->get['filter']) ){
			foreach(explode(',', $this->request->get['filter']) as $filter){
				if((int)$filter > 0){
					$filter_ids[] = (int)$filter;
				}
			}
		}
		...
		$price_min = $lowest_price = $this->model_catalog_product->getLowesetPrice($paths, $manufacturer_ids, $filter_ids);
		$price_max = $highest_price = $this->model_catalog_product->getHighestPrice($paths, $manufacturer_ids, $filter_ids);

		if($price_min == ''){
			$price_min = $lowest_price = 0;
		}
		if($price_max == ''){
			$price_max = $highest_price = 0;
		}
		// if no value of price min and price max in url request, 
		// then convert its value to the currency, 
		// else use back the converted value (to prevent double conversion)
		
		if( isset($this->request->get['price_min']) && (float)$this->request->get['price_min'] > -1 && $price_min > 0){
		...
		if( isset($this->request->get['price_max']) && (float)$this->request->get['price_max'] > -1 && $price_max > 0){
