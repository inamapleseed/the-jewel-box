...
class ControllerProductSearch extends Controller {
	public function index() {
		...
		//if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			$ori_total = (int)($product_total / $limit);
	    
            // $results = $this->model_catalog_product->getProducts($filter_data);
			$results = $this->model_catalog_product->getProductsTrial($filter_data);
            //if(count($results) < 15){
			    ...
			    $results2 = $this->model_catalog_product->getProductsTrial($filter_data);
    			$product_search = array();
    			foreach($results2 as $res){
    			    $product_search[] = $res['product_id'];
    			}
    			
    			$current_total_need = ($page * $limit);
    			$npage = (int)(($current_total_need - $product_total) / $limit);
    			if($product_total < $current_total_need && $npage == 0){
    			    $start  = 0;
    			    $left   = $current_total_need - $product_total;
    			    $npage  = 1;
    			}else{
    			    $left   = $limit;
    			    $npage  = (int)(($current_total_need - $product_total) / $limit);
    			    $start  = ((($ori_total+1) * $limit) - $product_total) + (($npage-1) * $limit);
    			}
    			
    			$category_infos = $this->db->query("SELECT category_id FROM ".DB_PREFIX."category_description WHERE name LIKE '%".$search."%'");
                foreach($category_infos->rows as $info){
                    $category_filter_data = array(
        				...
        				'start'               => $start,
        				'limit'               => $left,
        				'product_have'        => $product_search,
        			);
    			    $category_results = $this->model_catalog_product->getProductsTrial($category_filter_data);
        			foreach($category_results as $res){
        			    $product_search[] = $res['product_id'];
        			    if(count($results) < 15){
        			        $results[] = $res;
        			    }
        			}
        			$product_total += $this->model_catalog_product->getTotalProducts($category_filter_data);
                }
	...
	public function keyword() {
		...
		//if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
           ...
           // $results = $this->model_catalog_product->getProducts($filter_data);
			...
			$category_infos = $this->db->query("SELECT * FROM ".DB_PREFIX."category c INNER JOIN ".DB_PREFIX."category_description cd ON c.category_id = cd.category_id WHERE cd.name LIKE '%".$search."%' AND c.status = '1' LIMIT 10");
			$data['category'] = array();
			foreach($category_infos->rows as $category){
		        $data['category'][] = [
	                'name'  => $category['name'],
	                'url'   => $this->url->link('product/category','path='.$category['category_id'], true)
	            ];
			}
            ...
            $special = false;
			          ...
			          if ($this->customer->isLogged() || !$this->config->get('config_customer_price')){
                            if ($this->config->get('config_product_decimal_places')) {
                                $price = $this->currency->format($this->tax->calculate($res['price'], $res['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                            } else {
                                $price = $this->currency->format2($this->tax->calculate($res['price'], $res['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                            }
                        }
                        
                         if ((float)$res['special']) {
                                if ($this->config->get('config_product_decimal_places')) {
                                    $special = $this->currency->format($this->tax->calculate($res['special'], $res['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                                } else {
                                    $special = $this->currency->format2($this->tax->calculate($res['special'], $res['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                                }
                            }
			        ...
			        $data['res'][] = [
			                'image' => $this->model_tool_image->resize($res['image'], 40, 40),
			                'name'  => $res['name'],
			                'price' => ($special) ? $special :$price,
			                'retail' => $price,
			                'url'   => $this->url->link('product/product','product_id='.$res['product_id'], true)
