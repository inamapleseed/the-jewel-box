<?php
class ControllerProductReadyToWear extends Controller {

	public function index() {
        // Start products or gallery
        $this->session->data['main_product'] = "readytowear";
        ...
        $data['main_pcategories'] = $main_pcategories;
        // End products or gallery
		...
		foreach ($listing_conditions as $var => &$default){
			...
			if($var=='sort'){
				...
				if(isset($order) && isset($this->request->get['order']) && $order == $sort){
		...
		$data['breadcrumbs'][] = array(
			'text' => $data['heading_title_ready'],
			//'href' => $this->url->link('product/category') . $breadcrumbs_filter
			'href' => $this->url->link('product/ready-to-wear')
		);
			...
			foreach($categories as $category_id){
				...
				if($category_info){
					...
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('product/ready-to-wear', 'path=' . implode('_', $category_path) . $breadcrumbs_filter)
					);
				}
		...
		$filter_data = array(
			...
			'limit'              => $limit,				
			// Start products or gallery
			'filter_main_product' => $this->session->data['main_product'],	
			// End products or gallery				
		); // debug($filter_data);
		...
		$page_data = array(
			...
			'page'	=>	$page,
			'limit'	=>	$limit,
			'url'	=>	$this->url->link('product/ready-to-wear', $path . 'page={page}'. $pagination_filter),
		); 
