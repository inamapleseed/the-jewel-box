...
class ModelExtensionModuleTestimonial extends Model{
    ...
    public function addReview($data){
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', date_of_purchase = '" . $this->db->escape($data['date_of_purchase']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '0', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', date_added = NOW()");
    ...
    public function getReviews($start = 0, $limit = 20) {
        ...
        $query = $this->db->query("SELECT r.review_id, r.image, r.customer_id, r.author, r.rating, r.title, r.date_of_purchase, r.text, r.date_added FROM " . DB_PREFIX . "review r WHERE r.product_id = '0'  AND r.status = '1'  ORDER BY r.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);

        return $query->rows;
    }

    public function getReview($review_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "review r WHERE r.product_id = '0'  AND r.status = '1' AND review_id = '".(int)$review_id."' ");
