...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			return array(
				...
				'products_category'=> $query->row['products_category'],
	...
	public function getProductsTrial($data = array(), $sub_query = false) {
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				foreach ($words as $word) {
					$implode[] = "(( pd.name LIKE '%" . $this->db->escape($word) . "%' ) OR (pd.name LIKE '%" . $this->db->escape(str_replace("z","s",$word)) . "%')  OR (pd.name LIKE '%" . $this->db->escape(str_replace("Colour","Color",$word)) . "%') OR (pd.name LIKE '%" . $this->db->escape(str_replace("colour","color",$word)) . "%') OR (pd.name LIKE '%" . $this->db->escape(str_replace("3-in-1","in-1",$word)) . "%') OR (pd.name LIKE '%" . $this->db->escape(str_replace("2-in-1","in-1",$word)) . "%') OR (pd.name LIKE '%" . $this->db->escape(str_replace("In","in",$word)) . "%')) ";
				}
                ...
                //for more filter options (sku, model)start
			...
			if (!empty($data['filter_name'])) {
				...
				// Make filter related instead of exactly word ---- "LIKE '%" . xxx . "%'"
				// >> Related Options / Связанные опции
				// $sql .= " OR LCASE(p.sku) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				// $sql .= " OR LCASE(p.upc) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				// $sql .= " OR LCASE(p.ean) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				// $sql .= " OR LCASE(p.jan) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				// $sql .= " OR LCASE(p.isbn) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				// $sql .= " OR LCASE(p.mpn) REGEXP '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				
				
				// MATCH ANY CHAR
				$ar = explode(' ', utf8_strtolower($data['filter_name']));
				foreach($ar as $a) {
				    $sql .= " OR LCASE(p.sku) REGEXP '" . $this->db->escape($a) . "'";
    				$sql .= " OR LCASE(p.upc) REGEXP '" . $this->db->escape($a) . "'";
    				$sql .= " OR LCASE(p.ean) REGEXP '" . $this->db->escape($a) . "'";
    				$sql .= " OR LCASE(p.jan) REGEXP '" . $this->db->escape($a) . "'";
    				$sql .= " OR LCASE(p.isbn) REGEXP '" . $this->db->escape($a) . "'";
    				$sql .= " OR LCASE(p.mpn) REGEXP '" . $this->db->escape($a) . "'";
				}
		...
		if (isset($data['product_have'])) {
		    if (count($data['product_have']) > 0) {
			    $sql .= " AND p.product_id NOT IN (" . implode(",",$data['product_have']) . ")";
		    }
		}
	...
	public function getProducts($data = array(), $sub_query = false) {
		...
		// Start products or gallery
		if (!empty($data['filter_main_product'])) {
			$sql .= " AND p.products_category = '" . $data['filter_main_product'] ."'";
		}
		// End products or gallery
...
// Start products or gallery
			if ($this->session->data['main_product'] && isset($this->session->data['main_product'])) {
...
// End products or gallery
		...
		if ($paths || $manufacturer_ids || $filter_ids) { 
			...
			if($conds){
				...
				// Start products or gallery
				if ($main_pcategories) {
					$sub_query .= "AND p.products_category='". $main_pcategories ."'";
					// ' AND p.products_category='. $main_pcategories .''
				}
				// End products or gallery
		...
		if($sub_query){
			...
			// Start products or gallery
				...
				$sql = 'SELECT d.product_id, MIN(d.price) as price FROM (
					...
					SELECT product_id, MIN(a.price) as price FROM (SELECT product_id, price FROM `' . DB_PREFIX . 'product` UNION ALL SELECT product_id, price FROM `' . DB_PREFIX . 'product_special` WHERE customer_group_id="'.(int)$customer_group_id.'") a GROUP BY a.product_id
					) b 
					WHERE product_id IN 
					(
						SELECT p.product_id FROM `' . DB_PREFIX . 'product` as p WHERE p.products_category = "'. $main_pcategories .'"
					)
