...
class ControllerUserUserPermission extends Controller {
	...
	protected function getForm() {
		...
		if (!$this->user->is_dev() && $this->request->get['user_group_id'] == 1) {
			/* dev only */
		...
		$data['entry_is_dev'] = $this->language->get('entry_is_dev');
		$data['help_is_dev'] = $this->language->get('help_is_dev');
		...
		if (isset($this->request->post['is_dev'])) {
			$data['is_dev'] = $this->request->post['is_dev'];
		} elseif (!empty($user_group_info)) {
			$data['is_dev'] = $user_group_info['is_dev'];
		} else {
			$data['is_dev'] = 0;
		}
		...
		$ignore = array(
			...
			'common/common',
			'common/pagination',
			'event/d_twig_manager',
			'extension/module/pim',
            'insights/sales',
            'insights/analytics'
		);
		...
		while (count($path) != 0) {
			$next = array_shift($path); // debug($next);
