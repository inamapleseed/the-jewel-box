...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getForm() {
		...
		// Event Day Promotion - Start
		$data['entry_customer'] = $this->language->get('entry_customer');
		$data['help_customer'] = $this->language->get('help_customer');

		
		if (isset($this->request->post['coupon_customer'])) {
			$customerlist = $this->request->post['coupon_customer'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$customerlist = $this->model_marketing_coupon->getCouponCustomers($this->request->get['coupon_id']);
		} else {
			$customerlist = array();
		}
		...
		if (isset($this->request->post['coupon_brand'])) {
			$brands = $this->request->post['coupon_brand'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$brands = $this->model_marketing_coupon->getCouponManufacturers($this->request->get['coupon_id']);
		} else {
			$brands = array();
		}
		...
		$data['coupon_brand'] = array();

		foreach ($brands as $manufacturer_id) {
	...
	public function history() {
		...
		foreach ($results as $result) {
			...
			if (!$result['customer']) {
				...
				if ($order) {
					$result['customer'] = $order['firstname'] . ' ' . $order['lastname'];
				}
			}
