...
class ControllerCatalogPagebanner extends Controller {
	...
	protected function getRouteSets(){
		...
		$static = array(
			...
			'gallery/album'				=>	'Our Services',
			'gallery/gallery'			=>	'Our Services Inner',
	...
	public function getCategories(){
		$query = $this->db->query('SELECT DISTINCT cd.category_id, cd.name FROM `'. DB_PREFIX . 'category_path` cp LEFT JOIN  `'. DB_PREFIX . 'category_description` cd  ON (cd.category_id = cp.category_id) WHERE cp.path_id = cp.category_id AND cd.language_id="'.(int)$this->config->get('config_language_id').'" ORDER BY cd.name ASC');
		return $query->rows;
	}
