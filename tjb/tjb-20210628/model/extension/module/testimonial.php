...
class ModelExtensionModuleTestimonial extends Model {
	...
	public function addReview($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', date_of_purchase = '" . $this->db->escape($data['date_of_purchase']) . "', title = '" . $this->db->escape($data['title']) . "', image = '" . $this->db->escape($data['image']) . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "'");
	...
	public function editReview($review_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', date_of_purchase = '" . $this->db->escape($data['date_of_purchase']) . "', image = '" . $this->db->escape($data['image']) . "', title = '" . $this->db->escape($data['title']) . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = NOW() WHERE review_id = '" . (int)$review_id  . "'");
	}
