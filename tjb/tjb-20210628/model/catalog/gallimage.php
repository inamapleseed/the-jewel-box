...
class ModelCatalogGallimage extends Model {
	public function addGallimage($data) {		
		...
		if(isset($data['product_category'])){
			$this->db->query("DELETE FROM " . DB_PREFIX . "gallery_to_category WHERE gallery_id = '" . (int)$gallimage_id . "'");
			foreach ($data['product_category'] as $cat) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "gallery_to_category SET gallery_id = '" . (int)$gallimage_id . "', category_id = '" . (int)$cat . "'");
			}
		}
