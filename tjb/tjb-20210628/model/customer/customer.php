...
class ModelCustomerCustomer extends Model {
	...
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', birthday = '" . $this->db->escape($data['birthday']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
		...
		if (isset($data['address'])) {
			...
			foreach ($data['address'] as $address) {
				...
				if(!$index){
					$first_address_id = $this->db->getLastId();
				}
	...
	public function editCustomer($customer_id, $data) {
		...
		// newsletter module
		$this->handleNewsletter($data, $customer_id);
		// newsletter module
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', birthday = '" . $this->db->escape($data['birthday']) . "',  custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getCustomers($data = array()) {
		...
		(SELECT SUM(points) FROM " . DB_PREFIX . "customer_reward cr WHERE c.customer_id = cr.customer_id) AS reward
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			return array(
				...
				'unit_no'      	 => $address_query->row['unit_no'],
	...
	public function deleteReward($customer_reward_id) {
		//	$this->db->query("DELETE FROM " . DB_PREFIX . "customer_reward WHERE order_id = '" . (int)$order_id . "' AND points > 0");
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_reward WHERE customer_reward_id = '" . (int)$customer_reward_id . "' AND points > 0");
	}
	...
	private function handleNewsletter($data, $customer_id = 0) {
