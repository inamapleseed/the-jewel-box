...
class ModelDesignBanner extends Model {
	...
	public function editBanner($banner_id, $data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', language_id = '" . (int)$language_id . "', title = '" .  $this->db->escape($banner_image['title']) . "', btn_pos = '" .  $this->db->escape($banner_image['btn_pos']) . "', title_text = '" .  $this->db->escape($banner_image['title_text']) . "', title_font_size = '" .  $this->db->escape($banner_image['title_font_size']) . "', description = '" .  $this->db->escape($banner_image['description']) . "', link_text = '" .  $this->db->escape($banner_image['link_text']) . "', link = '" .  $this->db->escape($banner_image['link']) . "', mobile_image = '" .  $this->db->escape($banner_image['mobile_image']) . "', color_theme = '" .  $this->db->escape($banner_image['color_theme']) . "', mobile_color_theme = '" .  $this->db->escape($banner_image['mobile_color_theme']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '" . (int)$banner_image['sort_order'] . "'");
				}
			}
		}
	}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			$banner_image_data[$banner_image['language_id']][] = array(
				...
				'title_text'      => $banner_image['title_text'],
				'title_font_size'      => $banner_image['title_font_size'],
				'btn_pos'      => $banner_image['btn_pos'],
